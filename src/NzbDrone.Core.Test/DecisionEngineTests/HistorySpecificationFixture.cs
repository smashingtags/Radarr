    ...
    public class HistorySpecificationFixture : CoreTest<HistorySpecification>
    {
        ...
        public void Setup()
        {
            ...
            Mocker.GetMock<ICustomFormatCalculationService>()
                .Setup(x => x.ParseCustomFormat(It.IsAny<History.MovieHistory>()))
        ...
        private void GivenMostRecentForEpisode(int episodeId, string downloadId, QualityModel quality, DateTime date, MovieHistoryEventType eventType)
        {
            Mocker.GetMock<IHistoryService>().Setup(s => s.MostRecentForMovie(episodeId))
                  .Returns(new History.MovieHistory { DownloadId = downloadId, Quality = quality, Date = date, EventType = eventType });
        }
        ...
        {
            Mocker.GetMock<IHistoryService>().Setup(s => s.MostRecentForMovie(It.IsAny<int>())).Returns((History.MovieHistory)null);
